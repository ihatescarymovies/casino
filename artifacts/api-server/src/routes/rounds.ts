...
router.get("/:id/fairness", async (req: Request, res: Response) => {
  ...
  const [round] = await db.select().from(schema.gameRoundsTable).where(eq(schema.gameRoundsTable.id, roundId));
  if (!round) throw new GameRoundError(`Round ${roundId} not found`, 404);
  if (round.userId !== userId) { res.status(403).json({ error: "Forbidden" }); return; }
  const [chain] = round.serverSeedHash
    ? await db.select().from(schema.hashChainsTable).where(eq(schema.hashChainsTable.serverSeedHash, round.serverSeedHash))
    : [];
  const resolved = round.status !== "pending" && round.result !== "pending";
  res.status(200).json({
    roundId: round.id, gameType: round.gameType, status: round.status, demo: round.isDemo ?? false,
    commitment: round.serverSeedHash, clientSeed: round.clientSeed, nonce: round.nonce,
    chain: chain ? { id: chain.id, previousHash: chain.previousHash } : null,
    serverSeed: resolved ? chain?.serverSeed ?? null : null,
    params: round.details, result: resolved ? round.result : null, payout: resolved ? round.payout : null,
    algorithm: { name: "commitment-sha256", version: "1", replay: false },
  });
});

/* ── POST /api/rounds/:id/verify — Verify fairness ─────────────────── */
router.post("/:id/verify", validate(VerifyRoundBody), async (req: Request, res: Response) => {
  const userId = requireAuth(req, res);
  if (!userId) return;
  const roundId = Number(req.params.id); // URL is canonical; body roundId is ignored.
  if (!Number.isSafeInteger(roundId) || roundId < 1) { res.status(400).json({ error: "Invalid round ID" }); return; }
  const [round] = await db.select().from(schema.gameRoundsTable).where(eq(schema.gameRoundsTable.id, roundId));
  if (!round) throw new GameRoundError(`Round ${roundId} not found`, 404);
  if (round.userId !== userId) { res.status(403).json({ error: "Forbidden" }); return; }
  if (round.status === "pending" || round.result === "pending") { res.status(409).json({ error: "Pending rounds cannot be verified or reveal a seed" }); return; }

  const { serverSeed } = req.body;
  if (typeof serverSeed !== "string" || serverSeed.length < 1 || serverSeed.length > 128) { res.status(400).json({ error: "Invalid server seed" }); return; }
  const computedHash = createHash("sha256").update(serverSeed, "utf8").digest("hex");
